...
function Products() {
  ...
  const getProducts = async () => {
    try {
      const { data } = await Axioscall("post", "/general/products", null, true);
  ...
  const handleEdit = (item) => {
    setFormData({
      ...
      subCategory: item.subCategory[0]?._id || "",
      isActive: item.isActive,
      files: [],
      brand: item.brand[0]?._id || "",
      description: item.description,
      store: item.store[0]?._id || "",
    });
    setEditId(item._id);
    setIsEditing(true);
  ...
  const getAllBrands = async () => {
    try {
      const { data } = await Axioscall("get", "/seller/brand", null, true);
      setBrands(data.data);
    } catch (error) {
  ...
  const getStore = async () => {
    try {
      const { data } = await Axioscall("get", "/seller/store", null, true);
      setStores(data.data);
    } catch (error) {
  ...
  const submit = async () => {
    try {
      ...
      const url = isEditing ? `/general/product/${editId}` : "/general/product";
      const method = isEditing ? "put" : "post";

      await Axioscall(method, url, formDataData, true);
                    ...
                    value={
                      formData.store
                        ? {
                            ...
                            label: stores.find((s) => s._id === formData.store)
                              ?.store,
                          }
                        : null
                    }
                  ...
                  value={
                    formData.brand
                      ? {
                          ...
                          label: brands.find((b) => b._id === formData.brand)
                            ?.brand,
                        }
                      : null
                  }
